# PGND experiment notes (archived before repository simplification)

This is the former root README, preserved as a historical record. Its results,
scientific caveats and proposed methods have not been removed. Historical
commands/model menus and statements about what was "current" are not today's
operating instructions; use the [root README](../README.md) instead. Paths in
the retained text are relative to the repository root.

Normal training now supports PGND, LSTM, GRU, RNN and CNN-GRU only. The existing
`src.evaluate` CLI still loads all saved pilot checkpoints. Direct-only,
history-readout and modal definitions are frozen in `pgnd_experiments.py`;
PGND's observation-readout flag remains only for historical compatibility.
Exact pilot training sources remain in the corresponding local `results/*/source/`
snapshots. Those ignored artifacts must be backed up separately from Git.

---

# Physics-Guided Neural Dynamics for Pantograph–Catenary Contact Force Estimation

**PGND** is a research proof-of-concept for estimating contact force from panhead acceleration and displacement using physics-guided continuous-time latent dynamics. The code implements the original quadratic-potential, latent-only model (PGND-0 in the [manuscript](manuscript/main.tex)), an optional observation-conditioned readout, its direct-only control, and PyTorch RNN, LSTM, GRU, and CNN–GRU baselines. An optional modal-memory candidate and full-history controls are described below; they do not replace PGND-0 or the manuscript formulation. Warm-up initialization remains unimplemented. Results concern finite-window, next-sample prediction, not streaming or robustness validation; the manuscript's result section remains provisional.
...
During method development, add `--split validation` to evaluate only validation
predictions and save the same metrics/provenance as the test pipeline. The
default remains `--split test`; CSVs explicitly record the evaluated split.
Validation is for selection, not a substitute for independent confirmation.
...
## Research candidate: damped modal memory (not the default PGND)

The 16-sample input has only 15 ms between first and last observations, with
the target at 16 ms. A 20-Hz cycle lasts 50 ms; 64-sample windows are a modest
context probe, not necessarily sufficient for lower-frequency behavior.
The cutoff does not establish the signal's dominant frequency. Longer context
must be supplied to the baselines too, with identical eligible target rows.

The literature suggests testing structured memory rather than assuming a larger
neural ODE is better. [S4](https://arxiv.org/abs/2111.00396) motivates efficient
continuous-time state-space memory;
[LinOSS](https://arxiv.org/abs/2410.03943) uses forced oscillatory dynamics;
[D-LinOSS](https://arxiv.org/abs/2505.12171) learns damping separately from the
oscillation time scale. [Phy-SSM](https://arxiv.org/abs/2507.10792) incorporates
partially known physical dynamics. Its physical constraints cannot simply be
transferred here: our files contain no calibrated simulator state matrices.
None of these papers establishes an improvement on our contact-force dataset.

`--model pgnd_modal` implements a small **candidate inspired by** those ideas,
not a reproduction of their full architectures or a claim of a novel method.
For each mode, with physical time in seconds, it uses

```text
u_i(t) = encoder_i(standardized acceleration, standardized displacement)
q_i'' + 2*a_i*q_i' + (a_i^2 + w_i^2)*q_i = (a_i^2 + w_i^2)*u_i(t)
a_i = exp(log_decay_i), w_i = 2*pi*exp(log_frequency_i)
p_i = q_i' / sqrt(a_i^2 + w_i^2)
predicted standardized force = MLP([q_target, p_target, last_observation])
```

For an unforced mode, `E=(q^2+p^2)/2` satisfies `E'=-2*a*p^2 <= 0`.
This is latent stability, not calibrated physical energy or a force-balance
equation. Exact matrix-exponential transitions integrate piecewise-constant
observations; a vectorized sum gives the final state without RK4 stages or a
Python loop over time. The initial state is local equilibrium `q=u_0,p=0`,
and input k acts over `[t_k,t_(k+1))`, including the last prediction interval.
No observation at the target time and no force history is provided.

The declared pilot uses 32 modes, encoder 2->32->32, decoder 66->32->1, tanh,
3,393 trainable parameters, frequencies initialized log-spaced 1--50 Hz, and
initial `a=0.25*w` (subsequently independent). These are explicit assumptions,
not identified vibration modes. Only underdamped modes are represented.
Compared with PGND-0 this changes dense operators to diagonal modes, removes
the state-dependent neural residual and its penalty, changes linear
interpolation to zero-order hold, and changes initialization/readout. It is
not an equivalent numerical optimization of the original model.

Two additional controls receive exactly the same history: `history_linear`
is `2L->1`; `history_mlp` is `2L->32->32->1` with tanh. At L=64 they have
129 and 5,217 parameters. These controls help distinguish a dynamical benefit
from a simple full-history mapping. No new dependency or source module is added.

### Validation-only pilot

The declared protocol and exact launch/check scripts live under
`results/20hz_modal_pilot_20260925/` locally and on the new remote endpoint
(SSH port 38688). Seven independent jobs use 100 epochs, seed 0, the previous
Adam settings, batch 128, stride 16, and unchanged raw data/splits/scalers:
the modal candidate at L=16 and L=64; LSTM, GRU, CNN-GRU, history-linear and
history-MLP at L=64. Candidate modal width is explicitly set to 32 using
`--latent-dim 32`; the general CLI default remains 16 for original PGND.
Both final and best-validation checkpoints are retained. Only validation
predictions are evaluated; there is no new test-based architecture selection.
The original PGND implementation, default and manuscript remain unchanged.

A predeclared 5% validation-RMSE reduction against the strongest same-context
control is a practical screening threshold, not statistical significance.
If it is absent, do not promote the candidate. Validation shares recording
identities with training and is used to select checkpoints. Even a promising
pilot needs multiple seeds and whole-recording validation, followed by fresh
held-out simulation confirmation. Cross-length aggregate scores use different
target counts and must not be called a matched architecture-only comparison.

All seven remote pilot runs completed their 100-epoch budgets. The saved
checkpoints were reloaded and evaluated on **validation only**:

| Model (64 observations) | Selected epoch | Validation RMSE (N) |
| --- | ---: | ---: |
| CNN-GRU | 99 | 8.151499 |
| LSTM | 90 | 8.201116 |
| GRU | 95 | 8.269024 |
| History MLP | 97 | 8.532315 |
| Modal PGND candidate | 90 | 8.549341 |
| History linear | 81 | 12.031719 |

These six models share all 33,119 validation targets. The candidate is
**4.88% worse** than CNN-GRU, failing the predeclared improvement criterion.
At 16 observations it also loses to original PGND: 10.258478 versus 9.947902 N
validation RMSE on the same 33,983 targets. Do not promote the candidate or
rewrite the manuscript around it. It remains an optional, documented negative
experiment; original PGND and all legacy baselines remain unchanged.

On the 33,119 common validation rows, extending the history from 16 to 64
samples changes LSTM RMSE from 10.387238 to 8.201116 N, GRU from 10.185625 to
8.269024 N, and CNN-GRU from 9.914574 to 8.151499 N. This supports investigating
context before attributing gains to physics. The longer-context training set
has 12,072 instead of 12,126 targets; this is a protocol/context comparison,
not a controlled isolation of architecture. Original PGND at L=64 was not run.
No new held-out test metrics or statistical-significance claims follow from
this single-seed validation pilot.

Artifacts in the run directory include all 14 checkpoints, complete histories,
settings, logs, original/executed source snapshots, `validation_L16/`,
`validation_L64/`, and `audit.json`. Audits verified raw-data/source hashes,
training-only statistics, matching target rows, checkpoint selection and
metrics recomputed from predictions. Synthetic CUDA FP32/FP64 checks matched
the analytic transition, state and gradients to independent matrix-exponential
recurrences, and checked equilibrium, unforced stability and variable lengths.
The modal runs took about 125--126 seconds each including validation under
concurrent load; these are not isolated cross-architecture speed benchmarks.
No local training, no new test scoring, and no manuscript edits were performed.

### Recommended next formulation: a physically anchored force observer

This is a **proposal, not an implemented or proven improvement**. The present
PGND imposes a mechanical-looking latent structure but neither identifies
physical states nor constrains the force output by known mechanical balance.
That makes its physical advantage a hypothesis, not a consequence of PSD
damping. A more substantive next step is prediction-correction state estimation,
inspired by [KalmanNet](https://arxiv.org/abs/2107.10043) and the partially known
dynamics formulation of Phy-SSM, rather than another generic latent readout.
KalmanNet's original assumptions include available state labels and partially
known transition/observation models; our force-only supervision is not a
drop-in reproduction. The recent
[Koopman-aided Blind-KalmanNet preprint](https://arxiv.org/abs/2609.24763)
also explores learned predictors and gains when dynamics are unknown, but its
reported success does not establish identifiable physical states here.

A task-specific hypothesis is to augment a reduced mechanical state with the
unknown force and use a small learned correction only for unmodeled dynamics:

```text
s_k^- = Phi_mechanical+residual(s_(k-1)^+, dt)
s_k^+ = s_k^- + gain_theta(s_k^-, x_k) * (x_k - h_mechanical(s_k^-))
F_hat_m = force_component(Phi_mechanical+residual(s_(m-1)^+, dt))
```

Here `x_k` contains measured acceleration/uplift; `h_mechanical` links those
signals to mechanical states and the augmented force. The final step has no
correction using `x_m`, which is unavailable in our next-sample task. The
force-component evolution law, observability, initialization and the neural
gain must be specified and tested, not assumed. Training could combine force
prediction with measurement-reconstruction consistency on available historical
observations, using training-only scaling and validation-selected weights.

Before coding this physical version, obtain the simulator's reduced pantograph
equations, masses/stiffness/damping, suspension and uplift/aerodynamic forces,
sensor locations, units/sign conventions, and filtering procedure. Establish
which states/parameters are observable from the two measured channels; if
parameters are estimated, use training data only and check identifiability.
Do not fabricate a physical residual on arbitrary latent coordinates. A fair
study must include the mechanical observer alone, a learned observer without
the mechanical constraints, matched-context neural baselines, multiple seeds,
and genuinely new held-out cases. This is the recommended research direction,
not a promise that it will outperform the existing models.
